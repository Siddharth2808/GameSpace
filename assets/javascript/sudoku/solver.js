  ...
  if (clicked_id == "solve-button") {
    for (let i = 0; i < 81; i++) {
      if (cells[i].innerText != "")
        output.push(parseInt(cells[i].innerText, 10));
      else output.push(-1);
    }
    if (cellNow != -1)
  ...
  if (checksyntex() == true) {
    if (solve_sudoku() == true) {
      document.getElementById("result").innerText =
        "Yay sucessfully able to find solution";
